...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      setIconState() {
        if (this.unviewedNotifications.length > 0 && this.$store.state.notification.unviewed === false) {
          this.$store.dispatch("notification/setUnviewed", true)
        } else if (this.unviewedNotifications.length <= 0 && this.$store.state.notification.unviewed === true) {
